...
function computeStats(checkins: Checkin[], plan: WeeklyPlan | null) {
  const avgStress =
    checkins.length > 0
      ? (
...
export default function WeeklyPage() {
  ...
  const [lastWeekCollapsed, setLastWeekCollapsed] = useState(false);
  ...
  // This week stats
  const thisWeekStats = computeStats(checkins, plan);
  const thisWeekRange = formatWeekRange(weekStart);

  // Last week stats
  const lastWeekStats = lastWeek
    ? computeStats(lastWeek.checkins, lastWeek.plan)
    : null;
  const lastWeekRange = lastWeek ? formatWeekRange(lastWeek.weekStart) : null;

  // Group schedule by date for this week
  ...
  // Stress pattern from last week's checkins
  const lastWeekStressPattern = lastWeek?.checkins
    .filter((c) => c.stress_level !== null)
    .map((c) => ({
      date: new Date(c.date + "T12:00:00").toLocaleDateString("en-US", { weekday: "short" }),
  ...
  return (
    <div className="max-w-2xl space-y-8">
      ...
      {/* ═══════════════════════════════════════════════════════ */}
      {lastWeek && (lastWeek.plan || lastWeek.checkins.length > 0) && (
        <div className="space-y-4">
          <button
            onClick={() => setLastWeekCollapsed(!lastWeekCollapsed)}
            className="flex items-center gap-2 w-full text-left group"
          >
            <svg
              className={`w-4 h-4 text-neutral-400 transition-transform ${
                lastWeekCollapsed ? "" : "rotate-90"
              }`}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            ...
            <div>
              <p className="text-xs font-medium text-neutral-400 dark:text-neutral-500 uppercase tracking-widest">
                {lastWeekRange?.start} — {lastWeekRange?.end}
              </p>
              <h2 className="text-xl font-bold text-neutral-900 dark:text-neutral-100 group-hover:text-neutral-600 dark:group-hover:text-neutral-300 transition-colors">
                Last Week Review
              </h2>
            </div>
          </button>
          ...
          {!lastWeekCollapsed && (
            <div className="space-y-4 pl-6 border-l-2 border-neutral-200 dark:border-neutral-700">
              ...
              {lastWeekStats && (
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  <div className="p-4 rounded-xl bg-neutral-50 dark:bg-neutral-800/50">
                    ...
                    <p className="text-lg font-bold text-neutral-900 dark:text-neutral-100">
                      {lastWeekStats.completedTasks}/{lastWeekStats.totalTasks}
                      <span className="text-xs font-normal text-neutral-400 ml-1">
                        ({lastWeekStats.completionPct}%)
                      </span>
                    </p>
                  </div>
                </div>
              ...
              {/* Cycle Completion for last week */}
              {lastWeek.plan && lastWeekStats && lastWeekStats.totalTasks > 0 && (
                <div className="p-4 rounded-xl border border-neutral-200 dark:border-neutral-700">
                  ...
                  <div className="w-full bg-neutral-200 dark:bg-neutral-700 rounded-full h-2.5 mb-2">
                    <div
                      ...
                      style={{ width: `${lastWeekStats.completionPct}%` }}
                    />
                  </div>
                  <p className="text-xs text-neutral-500">
                    {lastWeekStats.completedTasks} of {lastWeekStats.totalTasks} tasks completed
                    {lastWeekStats.completionPct === 100
                      ? " — full cycle completed!"
                      : lastWeekStats.completionPct >= 75
                        ? " — strong finish"
                        : lastWeekStats.completionPct >= 50
                          ? " — decent progress"
                          : " — some tasks left open"}
                  </p>
                </div>
              )}

              {/* Stress patterns */}
              {lastWeekStressPattern.length > 0 && (
                <div className="p-4 rounded-xl border border-neutral-200 dark:border-neutral-700">
                  <h3 className="text-sm font-semibold text-neutral-900 dark:text-neutral-100 mb-3">
                    Stress Pattern
                  </h3>
                  <div className="flex items-end gap-2">
                    {lastWeekStressPattern.map((s, i) => (
              ...
              <div className="p-4 rounded-xl border border-neutral-200 dark:border-neutral-700">
                ...
                {!summary && !loadingSummary && (
                  <p className="text-xs text-neutral-400">
                    Click &quot;Generate&quot; for AI insights on last week&apos;s patterns.
                  </p>
                )}
              </div>

              {/* Stressor suggestions from last week's plan */}
              {lastWeek.plan?.ai_suggestions && lastWeek.plan.ai_suggestions.length > 0 && (
                <div className="p-4 rounded-xl border border-neutral-200 dark:border-neutral-700">
                  <h3 className="text-sm font-semibold text-neutral-900 dark:text-neutral-100 mb-3">
                    Stressor Suggestions
                  </h3>
                  <div className="space-y-2">
                    {lastWeek.plan.ai_suggestions.map((s, i) => (
                      <div
                        key={i}
      ...
      <div className="space-y-6">
        ...
        {/* Plan exists — show this week's content */}
        {plan && (
          <>
            {/* Intention */}
            {plan.intention && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div className="p-4 rounded-xl bg-neutral-50 dark:bg-neutral-800/50">
                ...
                <p className="text-lg font-bold text-neutral-900 dark:text-neutral-100">
                  {thisWeekStats.completedTasks}/{thisWeekStats.totalTasks}
                  <span className="text-xs font-normal text-neutral-400 ml-1">
                    ({thisWeekStats.completionPct}%)
                  </span>
                </p>
              </div>
            </div>
            ...
            {/* Current week stressor suggestions */}
